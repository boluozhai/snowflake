...
import com.boluozhai.snowflake.xgit.vfs.impl.objectbank.pack.PackDataReader;

public class PackDataReaderTest {
	...
	public void test() {
		...
		try {
			...
			file = file.child("pack-test.pack");

			PackDataReader reader = new PackDataReader(file);
			for (;;) {
				int res = reader.read();
				if (res < 0) {
					break;
